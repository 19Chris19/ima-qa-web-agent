...
运行中的服务通过“受控浏览器接入”保存账号登录态。优先打开 `/admin.html`，输入管理员 token 后点击“接入账号”：填写名称后，服务默认在后台创建独立 IMA 浏览器资料，并直接在管理页显示一次性二维码；**每次接入只接受待接入账号扫描二维码，不使用维护机已登录微信的快捷登录。** 扫码成功后自动验证共享知识库、加密保存凭证并刷新账号池。

若 IMA 或微信嵌入页暂时无法把二维码回传到管理页，任务不会马上作废。管理页会显示脱敏的失败阶段和耗时，并提供“打开受控登录窗口”：此时才会启动可见窗口，在该窗口继续扫码即可，服务仍会自动检测登录、验证并关闭窗口。二维码、临时浏览器 profile 和未成功的登录状态不会保留，运行期也不依赖浏览器窗口。

账号池按扫码后取得的 IMA 登录身份做服务端不可逆指纹去重，不以维护者填写的名称作为身份依据。同一 IMA 账号不能通过更换名称占用额外并发；旧账号库中发现的重复身份会自动停用并在管理页标记，保留原记录供维护者确认后删除。

页面二维码接入需要 **运行 Node 服务的维护机** 能访问 Chrome、Chromium 或 Ego Lite；可以用 `IMA_WEB_AGENT_BROWSER_PATH` 指定可执行文件。默认 Docker 容器不自带桌面浏览器，因此容器部署请在有图形界面的维护机使用下方 CLI + SSH 隧道方式，或让 Provider A 原生运行在维护机上完成接入。
